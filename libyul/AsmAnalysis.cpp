...
std::vector<YulString> AsmAnalyzer::operator()(Identifier const& _identifier)
{
	...
	auto watcher = m_errorReporter.errorWatcher();
	YulString type = m_nameRepository.dialect().defaultType;
...
void AsmAnalyzer::operator()(VariableDeclaration const& _varDecl)
{
	...
	if (_varDecl.value)
	{
		...
		{
			YulString givenType = m_nameRepository.dialect().defaultType;
			if (i < types.size())
				givenType = types[i];
...
std::vector<YulString> AsmAnalyzer::operator()(FunctionCall const& _funCall)
{
	...
	if (watcher.ok())
	{
		...
		return std::vector<YulString>(returnTypes->size(), m_nameRepository.dialect().defaultType);
	else
		return {};
}
