	...
	::createStackLayout(
		...
		[&](unsigned _i)
		{
			...
			else
			{
				...
				std::string msg =
					"Cannot swap " + (varNameDeep.empty() ? "Slot " + stackSlotToString(deepSlot, *m_dfg.nameRepository) : "Variable " + varNameDeep.str()) +
					" with " + (varNameTop.empty() ? "Slot " + stackSlotToString(m_stack.back(), *m_dfg.nameRepository) : "Variable " + varNameTop.str()) +
					": too deep in the stack by " + std::to_string(deficit) + " slots in " + stackToString(m_stack, *m_dfg.nameRepository);
		...
		[&](StackSlot const& _slot)
		{
			...
			if (auto depth = util::findOffset(m_stack | ranges::views::reverse, _slot))
			{
				...
				else if (!canBeFreelyGenerated(_slot))
				{
					...
					std::string msg =
						(varName.empty() ? "Slot " + stackSlotToString(_slot, *m_dfg.nameRepository) : "Variable " + varName.str())
						+ " is " + std::to_string(*depth - 15) + " too deep in the stack " + stackToString(m_stack, *m_dfg.nameRepository);
					m_stackErrors.emplace_back(StackTooDeepError(
						m_currentFunctionInfo ? m_currentFunctionInfo->function.name : YulString{},
						varName,
						deficit,
