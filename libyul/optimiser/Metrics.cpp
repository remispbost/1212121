...
size_t CodeCost::codeCost(YulNameRepository const& _nameRepository, Expression const& _expr)
{
	CodeCost cc(_nameRepository);
	cc.visit(_expr);
	return cc.m_cost;
}
...
void CodeCost::operator()(FunctionCall const& _funCall)
{
	...
	if (auto instruction = toEVMInstruction(m_nameRepository, _funCall.functionName.name))
	{
...
{
	evmasm::Tier gasPriceTier = evmasm::instructionInfo(_instruction, evmVersionFromDialect(m_nameRepository.dialect())).gasPriceTier;
