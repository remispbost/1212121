...
TestCase::TestResult YulOptimizerTest::run(std::ostream& _stream, std::string const& _linePrefix, bool const _formatted)
{
	...
	auto const printed = (result->subObjects.empty() ? AsmPrinter{ AsmPrinter::Mode::OmitDefaultType, result->code->nameRepository() }(result->code->block()) : result->toString(*m_dialect));
