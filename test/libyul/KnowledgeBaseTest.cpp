...
protected:
	KnowledgeBase constructKnowledgeBase(std::string const& _source)
	{
		...
		OptimiserStepContext context{m_dialect, nameRepository, dispenser, reserved, 0};
		...
		return KnowledgeBase([this](YulString _var) { return util::valueOrNullptr(m_values, _var); }, m_object->code->nameRepository());
	}
