...
)
{
	...
	if (_optimiserSettings.runYulOptimiser && _localVariables.empty())
	{
		...
		if (_system)
		{
			...
			solAssert(m_generatedYulUtilityCode.empty(), "");
			m_generatedYulUtilityCode = yul::AsmPrinter(yul::AsmPrinter::Mode::OmitDefaultType, obj.code->nameRepository())(obj.code->block());
			std::string code = yul::AsmPrinter{yul::AsmPrinter::Mode::OmitDefaultType, obj.code->nameRepository()}(obj.code->block());
...
void CompilerContext::optimizeYul(yul::Object& _object, OptimiserSettings const& _optimiserSettings, std::set<yul::YulString> const& _externalIdentifiers)
{
#ifdef SOL_OUTPUT_ASM
